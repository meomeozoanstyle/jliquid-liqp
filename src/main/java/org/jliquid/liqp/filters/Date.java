...
class Date extends Filter {
	...
	private final static java.util.Map<Character, SimpleDateFormat> LIQUID_TO_JAVA_FORMAT = new java.util.HashMap<Character, SimpleDateFormat>();
	 ...
	 * Reformat a date
	 * 
	 * %a - The abbreviated weekday name (``Sun'') %A - The full weekday name
	 * (``Sunday'') %b - The abbreviated month name (``Jan'') %B - The full
	 * month name (``January'') %c - The preferred local date and time
	 * representation %d - Day of the month (01..31) %H - Hour of the day,
	 * 24-hour clock (00..23) %I - Hour of the day, 12-hour clock (01..12) %j -
	 * Day of the year (001..366) %m - Month of the year (01..12) %M - Minute of
	 * the hour (00..59) %p - Meridian indicator (``AM'' or ``PM'') %S - Second
	 * of the minute (00..60) %U - Week number of the current year, starting
	 * with the first Sunday as the first day of the first week (00..53) %W -
	 * Week number of the current year, starting with the first Monday as the
	 * first day of the first week (00..53) %w - Day of the week (Sunday is 0,
	 * 0..6) %x - Preferred representation for the date alone, no time %X -
	 * Preferred representation for the time alone, no date %y - Year without a
	 * century (00..99) %Y - Year with century %Z - Time zone name %% - Literal
	 * ``%'' character
	 */
	@Override
	public Object apply(Object value, Object... params) {
		...
		try {
			...
			if (super.asString(value).equals("now")) {
				...
				// No need to divide this by 1000, the param is expected to be
				// in seconds already!
				seconds = super.asNumber(value).longValue();
			} else if (value instanceof java.util.Date) {
				seconds = ((java.util.Date) value).getTime() / 1000L;
			} else {
				seconds = trySeconds(super.asString(value)); // formatter.parse(super.asString(value)).getTime()
																// / 1000L;
			...
			for (int i = 0; i < format.length(); i++) {
				...
				if (ch == '%') {
					...
					SimpleDateFormat javaFormat = LIQUID_TO_JAVA_FORMAT
							.get(next);

					if (javaFormat == null) {
						// no valid date-format: append the '%' and the
						// 'next'-char
	...
	private static void init() {
		...
		LIQUID_TO_JAVA_FORMAT.put('c', new SimpleDateFormat(
				"EEE MMM dd HH:mm:ss yyyy", locale));
		...
		LIQUID_TO_JAVA_FORMAT
				.put('x', new SimpleDateFormat("MM/dd/yy", locale));

		// %X - Preferred representation for the time alone, no date
		LIQUID_TO_JAVA_FORMAT
				.put('X', new SimpleDateFormat("HH:mm:ss", locale));
	...
	/*
	 * Try to parse `str` into a Date and return this Date as seconds since
	 * EPOCH, or null if it could not be parsed.
	 */
